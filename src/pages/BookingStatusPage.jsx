...
export default function BookingStatusPage() {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      const qRef = query(collection(db, 'bookings'), where('bookingNumber', '==', trimmed.toUpperCase()));
      const qMobile = query(collection(db, 'bookings'), where('mobile', '==', trimmed));

      const [snapRef, snapMobile] = await Promise.all([
        getDocs(qRef),
        getDocs(qMobile)
      ]);

      const results = new Map();
      snapRef.forEach(doc => results.set(doc.id, { id: doc.id, ...doc.data() }));
      snapMobile.forEach(doc => results.set(doc.id, { id: doc.id, ...doc.data() }));

      if (results.size === 0) {
        setError('No booking found with that reference or phone number. Please double-check and try again.');
      } else {
    ...
    <div className="min-h-screen bg-navy-900 flex items-center justify-center p-6">
      ...
      <div className="w-full max-w-[560px]">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-white/50 text-sm">Enter your booking reference or phone number to view your reservation details.</p>
        </div>
          ...
          <form onSubmit={handleSearch} className="bg-navy-800 border border-white/10 rounded-2xl p-8 shadow-card">
            <div className="flex flex-col gap-2 mb-5">
              ...
              <input
                ...
                placeholder="BSH-XXXXXX or 09XXXXXXXXX"
        ...
        {bookings && bookings.length > 0 && (
          <div className="flex flex-col gap-6 animate-fade-in-up">
            ...
            <button
              onClick={handleReset}
              className="mt-2 w-full py-3 bg-white/5 hover:bg-white/10 border border-white/10 text-white font-semibold rounded-xl transition-colors text-sm"
            >
              Search Again
            </button>
          </div>
...
function BookingResultCard({ booking }) {
  ...
  return (
    <div className="bg-navy-800 border border-white/10 rounded-2xl overflow-hidden shadow-card">
      {/* Status Banner */}
